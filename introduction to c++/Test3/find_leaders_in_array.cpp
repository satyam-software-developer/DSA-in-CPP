
/* Problem statement
Given an integer array A of size n. Find and print all the leaders present in the input array. 
An array element A[i] is called Leader, if all the elements following it (i.e. present at its right) are less than or equal to A[i].

Print all the leader elements separated by space and in the reverse order. 
That means whichever leader comes at last should be printed first.

Detailed explanation ( Input/output format, Notes, Images )
Input Format :
Line 1 : Integer n, size of array
Line 2 : Array A elements (separated by space)
Output Format :
 leaders of array (separated by space)
Constraints :
1 <= n <= 10^6

Sample Input 1 :
6
3 12 34 2 0 -1
Sample Output 1 :
-1 0 2 34
Sample Input 2 :
5
13 17 5 4 6
Sample Output 2 :
6 17
*/

#include <iostream> // Include the input/output stream library for basic I/O operations
using namespace std; // Allow usage of standard namespace for standard identifiers

void leaders(int input[], int n) { // Define a function named 'leaders' that takes an array and its size as arguments
    int max = INT_MIN; // Initialize a variable 'max' to the smallest possible integer value
    for (int i = n - 1; i >= 0; i--) { // Iterate through the array from the end to the beginning
        if (input[i] >= max) { // Check if the current element is greater than or equal to 'max'
            cout << input[i] << " "; // Print the current element as it's a leader
            max = input[i]; // Update 'max' to the current element
        }
    }
}

int main() { // Define the main function, the entry point of the program
    int n; // Declare an integer variable 'n' to store the size of the array
    cin >> n; // Read the size of the array from the standard input

    int input[n]; // Declare an integer array 'input' with size 'n' provided by the user

    // Input loop: Read 'n' integers from the standard input and store them in the 'input' array
    for (int i = 0; i < n; i++) {
        cin >> input[i];
    }

    leaders(input, n); // Call the 'leaders' function passing the array 'input' and its size 'n'

    return 0; // Indicate successful termination of the program
}
