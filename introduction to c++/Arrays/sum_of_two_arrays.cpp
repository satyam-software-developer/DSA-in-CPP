...
void sumOfTwoArrays(int *input1, int size1, int *input2, int size2, int *output)
{
    ...
    // Iterate over both arrays from the end
    while (i >= 0 && j >= 0)
    {
        // Compute the sum of corresponding elements along with carry
        int sum = input1[i] + input2[j] + carry;
        
        // Store the least significant digit of the sum
        output[k] = sum % 10;
        
        // Update the carry for the next iteration
        carry = sum / 10;

        // Move to the next element in both arrays and output
        i--;
        j--;
        k--;
    }
    
    // Handle the remaining elements in the first array
    while (i >= 0)
    {
    ...
    // Handle the remaining elements in the second array
    while (j >= 0)
    {
    ...
    // Store the final carry in the most significant digit of the output
    output[0] = carry;
}
