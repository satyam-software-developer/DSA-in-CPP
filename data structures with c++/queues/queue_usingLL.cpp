...
private:
    ...
    Node* tail; // Tail (rear) of the queue
...
public:
    ...
    void enqueue(int data) {
        ...
        if (this->head == nullptr) { // If the queue is empty
    ...
    int dequeue() {
        ...
        Node* temp = this->head; // Temporarily store the front node
        this->head = this->head->next; // Move the head to the next node
        delete temp; // Delete the old head node
        this->size--; // Decrement the size of the queue
        if (this->head == nullptr) { // If queue becomes empty after dequeue
            this->tail = nullptr; // Reset tail to null
        }
...
int main() {
    int q;
    cin >> q; // Read the number of operations

    QueueUsingLL queue; // Create an instance of QueueUsingLL

    while (q > 0) {
        int choice;
        cin >> choice; // Read operation choice

        switch (choice) {
            case 1: {
                int input;
                cin >> input; // Read the element to enqueue
                queue.enqueue(input); // Enqueue the element
                break;
            }

            case 2:
                cout << queue.dequeue() << endl; // Dequeue and print the result
                break;

            case 3:
                cout << queue.front() << endl; // Get and print the front element
                break;

            case 4:
                cout << queue.getSize() << endl; // Get and print the size of the queue
                break;

            default:
                cout << (queue.isEmpty() ? "true" : "false") << endl; // Check if the queue is empty
                break;
        }
